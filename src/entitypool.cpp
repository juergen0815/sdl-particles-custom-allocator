...
public:
    void* Allocate( ) throw (std::bad_alloc,std::exception);

    void* Allocate( std::size_t num ) throw (std::bad_alloc,std::exception);

    void Free( void* p ) throw (std::bad_alloc,std::exception);

    void Free( void* p, std::size_t num ) throw (std::bad_alloc,std::exception);

    MemoryPool( std::size_t num, std::size_t size ) throw (std::bad_alloc,std::exception);

    MemoryPool( const MemoryPool& other ) throw (std::bad_alloc,std::exception);

    ~MemoryPool() throw (std::bad_alloc,std::exception);

};

MemoryPool::MemoryPool( std::size_t num, std::size_t size ) throw(std::bad_alloc,std::exception)
    : m_Data( nullptr )
    , m_NumBlocks(num)
    , m_BlockSize(size)
...
MemoryPool::MemoryPool( const MemoryPool& o ) throw(std::bad_alloc,std::exception)
...
MemoryPool::~MemoryPool() throw (std::bad_alloc,std::exception)
{
    if ( m_NextPool ) {
        delete m_NextPool;
    }
...
void* MemoryPool::Allocate() throw(std::bad_alloc,std::exception)
{
...
void* MemoryPool::Allocate( std::size_t num ) throw(std::bad_alloc,std::exception)
{
    // TODO: Make thread safe
...
void MemoryPool::Free( void* p ) throw(std::bad_alloc,std::exception)
{
    // TODO: Make thread safe
...
void MemoryPool::Free( void* p, std::size_t num ) throw(std::bad_alloc,std::exception)
{
    // TODO: Make thread safe
...
MemoryPool* EntityPool::CreatePool( std::size_t num, std::size_t objSize ) throw(std::exception)
{
    MemoryPool* pool = new MemoryPool( num, objSize);
    return pool;
}

void EntityPool::DestroyPool( MemoryPool *pool ) throw(std::exception)
{
    ASSERT( pool, "Invalid pool!" );
    delete pool;
}

void EntityPool::Free( MemoryPool* pool, Entity* p, std::size_t num ) throw(std::bad_alloc,std::exception)
{
    num > 1 ? pool->Free( p, num ) : pool->Free( p );
}

void EntityPool::Free( MemoryPool* pool, Entity* p ) throw(std::bad_alloc,std::exception)
{
    pool->Free( p );
}

Entity* EntityPool::Allocate( MemoryPool* pool, std::size_t num ) throw(std::bad_alloc,std::exception)
{
    void *p = nullptr;
    if ( num > 1 ) {
        p = pool->Allocate( num );
